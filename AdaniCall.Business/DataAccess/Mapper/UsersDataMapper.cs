    ...
    public class UsersDataMapper
    {
        ...
        public Users GetDetails(SqlDataReader sqlDataReader)
        {
            try
            {
                ...
                if (sqlDataReader.HasColumn(UsersDBFields.AirportShortName))
                    objUsers.AirportShortName = (sqlDataReader[UsersDBFields.AirportShortName] != DBNull.Value ? Convert.ToString(sqlDataReader[UsersDBFields.AirportShortName]) : string.Empty);
                if (sqlDataReader.HasColumn(UsersDBFields.DefaultName))
                    objUsers.DefaultName = (sqlDataReader[UsersDBFields.DefaultName] != DBNull.Value ? Convert.ToString(sqlDataReader[UsersDBFields.DefaultName]) : string.Empty);
                if (sqlDataReader.HasColumn(UsersDBFields.DefaultPhone))
                    objUsers.DefaultPhone = (sqlDataReader[UsersDBFields.DefaultPhone] != DBNull.Value ? Convert.ToString(sqlDataReader[UsersDBFields.DefaultPhone]) : string.Empty);
                if (sqlDataReader.HasColumn(UsersDBFields.DefaultEmail))
                    objUsers.DefaultEmail = (sqlDataReader[UsersDBFields.DefaultEmail] != DBNull.Value ? Convert.ToString(sqlDataReader[UsersDBFields.DefaultEmail]) : string.Empty);
                if (sqlDataReader.HasColumn(UsersDBFields.LevelTwoName))
                    objUsers.LevelTwoName = (sqlDataReader[UsersDBFields.LevelTwoName] != DBNull.Value ? Convert.ToString(sqlDataReader[UsersDBFields.LevelTwoName]) : "");
            }
            catch (Exception ex)
            {
