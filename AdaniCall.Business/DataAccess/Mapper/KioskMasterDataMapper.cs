    ...
    public class KioskMasterDataMapper
    {
        ...
        public KioskMaster GetDetails(SqlDataReader sqlDataReader)
        {
            try
            {
                ...
                if (sqlDataReader.HasColumn(KioskMasterDBFields.Terminal))
                    objKioskMaster.Terminal = (sqlDataReader[KioskMasterDBFields.Terminal] != DBNull.Value ? Convert.ToString(sqlDataReader[KioskMasterDBFields.Terminal]) : string.Empty);
                if (sqlDataReader.HasColumn(KioskMasterDBFields.RowNumber))
                    objKioskMaster.RowNumber = (sqlDataReader[KioskMasterDBFields.RowNumber] != DBNull.Value ? Convert.ToInt64(sqlDataReader[KioskMasterDBFields.RowNumber]) : 0);
                if (sqlDataReader.HasColumn(KioskMasterDBFields.LocationName))
                    objKioskMaster.LocationName = (sqlDataReader[KioskMasterDBFields.LocationName] != DBNull.Value ? Convert.ToString(sqlDataReader[KioskMasterDBFields.LocationName]) : string.Empty);
...
if (drow.Table.Columns.Contains(KioskMasterDBFields.DeviceModel)) 
  ...
  if (drow.Table.Columns.Contains(KioskMasterDBFields.Terminal))
   objKioskMaster.Terminal = (drow[KioskMasterDBFields.Terminal] != DBNull.Value ? Convert.ToString(drow[KioskMasterDBFields.Terminal]) : string.Empty);
