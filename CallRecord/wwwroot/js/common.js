        ...
        if (callingPage.toLowerCase().indexOf("accept") > -1) {
            $.ajax({
                ...
                data: {
                   ...
                   // alert("StartRecording serverid: " + serverid);
                }
            });
        }
    }
...
function GetAvailableAgent(txt) {
    ...
    if (callingPage.toLowerCase().indexOf("call") > -1) {
        $.ajax({
            ...
            success: function (data) {
                if (data.data.id > 0) {
                    //alert("Available Agent From: " + txt + " & " + data.data.agentCallID);
...
function GetKioskDetails(callerID) {
    ...
    if (callingPage.toLowerCase().indexOf("accept") > -1) {
        $.ajax({
            ...
            success: function (data) {
                if (data.data.id > 0) {
                    //alert("GetKioskDetails: " + data.data.deviceName)
...
function FreeAgent(agentCallerID, frm) {
    ...
    if (callingPage.toLowerCase().indexOf("accept") > -1) {
        $.ajax({
            ...
            async: false,
            success: function (data) {
                //alert("Free Agent from Accept: " + frm);
            },
            complete: function () {
                setTimeout(function () {
                }, 300);
        ...
        $.ajax({
            ...
            async: false,
            success: function (data) {
                //alert("Free Agent from Call: " + frm + "agentCallerID: " + agentCallerID);
            },
            complete: function () {
                setTimeout(function () {
                }, 300);
...
function MakeCallTransaction(uniqueCallID, incomingCallID) {
    ...
    if (callingPage.toLowerCase().indexOf("accept") > -1) {
        $.ajax({
            ...
            async: true,
            success: function (data) {
                //alert("MakeCallTransaction: "+"uniqueCallID: " + uniqueCallID + "incomingCallID: " + incomingCallID)
            },
            complete: function () {
                setTimeout(function () {
                }, 300);
...
function UpdateCallTransactionsEndTime(uniqueCallID) {
    ...
    if (callingPage.toLowerCase().indexOf("accept") > -1) {
        ...
        $.ajax({
            ...
            async: true,
            success: function (data) {
               // alert("UpdateCallTransactionsEndTime: " + "uniqueCallID: " + uniqueCallID + "callLanguage: " + callLanguage)
           },
            complete: function () {
                setTimeout(function () {
                    }, 300);
...
function InsertAccessMember(uniqueCallID, callStatus) {
   $.ajax({
        ...
        async: true,
       success: function (data) {
           //alert("InsertAccessMember: " + "uniqueCallID: " + uniqueCallID + "callStatus: " + callStatus)
             },
        complete: function () {
            setTimeout(function () {
                }, 300);
    ...
    if (callingPage.toLowerCase().indexOf("accept") > -1) {
        if (document.getElementById('telephoneRing') == null && document.getElementById('connectedLabel').hidden == true) {
            ...
            $.ajax({
                ...
                async: false,
                success: function (data) {
                    //alert("CheckIfAgentInactive:" +"MakeAgentActive: ")
                },
                complete: function () {
                    setTimeout(function () {
                     }, 300);
...
function feedback() {
    $.ajax({
        url: '/Home/GetFeedback', //***** from controller (Inside GetSentenceList called Partial View _ListSentencePartial)
        data: { "ID": 1 },
        type: "POST",
        cache: false,
        async: false,
...
var feedbackType = [];
function getFeedback(event, qType, inputId) {
    event.preventDefault();
    if (feedbackType.includes(qType) === false) {
        feedbackType.push(inputId);
        $("#feedbackVal").val(feedbackType);
    }
    console.log($("#feedbackVal").val());
    if (feedbackType.length != "") {
        $(".feedback-screen1").fadeOut(100);
        $(".feedback-screen2").fadeIn(1000);
        feedbackType = [];
    }
};
var feedbackMob = [];
function getFeedbackMob(event, qType, inputId) {
    if (feedbackMob.length <= 19) {
        event.preventDefault();
        if (feedbackMob.includes(qType) === false) {
            feedbackMob.push(inputId);
        }

        $("#mobileNum").val(feedbackMob.join(""));

        if (feedbackMob.length != 0) {
            $("#removeMobNum").fadeIn(500);
        }
    }
};

function removeMobileNum() {
    console.log("clicked ", feedbackMob.length);
    if (feedbackMob.length > 0) {
        console.log("Before ", feedbackMob);
        feedbackMob.pop();
        console.log("After ", feedbackMob);
        $("#mobileNum").val(feedbackMob.join(""));
        console.log($("#mobileNum").val(), feedbackMob.length);
        if (feedbackMob.length == 0) {
            $("#removeMobNum").fadeOut(100);
        }
    }
}
...
function saveFeedback() {
    ...
    $('#frmfeedback input').each(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.isSuccess) {
                //showBSAlert(data.MessageCaption, data.Message, __SUCCESS, true, "", true);
                $(".feedback-screen1").fadeOut(100);
                $(".feedback-screen2").fadeOut(100);
                $(".feedback-screen3").fadeIn(1000);
                setTimeout(function () {
